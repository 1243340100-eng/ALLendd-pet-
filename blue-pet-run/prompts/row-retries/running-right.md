Create Codex pet row `running-right` for `blue`: exactly 8 full-body frames in one horizontal strip on flat pure user-selected #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: round sky-blue octopus mascot with eight small round tentacles, large shiny blue eyes, pink cheek dots, and a soft crooked black wizard hat with a white band; friendly compact sticker-like paper-cut mascot style. Preserve silhouette, face, palette, material, proportions, markings, and props.
